...
bool _SetFullContextDataExLegacy(HANDLE hActiveThread, TITAN_ENGINE_CONTEXT_t* titcontext, bool AVX_PRIORITY)
{
    CONTEXT DBGContext;
...
{
    Thread* thread = emu.GetThreadFromHandle(hActiveThread);
    if (thread == nullptr)
        return _SetFullContextDataExLegacy(hActiveThread, titcontext, AVX_PRIORITY);

    CONTEXT& DBGContext = thread->registers.GetContextForModify();
...
static bool _GetFullContextDataExLegacy(HANDLE hActiveThread, TITAN_ENGINE_CONTEXT_t* titcontext, bool avx)
{
    CONTEXT DBGContext;
...
bool _GetFullContextDataEx(HANDLE hActiveThread, TITAN_ENGINE_CONTEXT_t* titcontext, bool avx)
{
    Thread *thread = emu.GetThreadFromHandle(hActiveThread);
    if(thread == nullptr)
        return _GetFullContextDataExLegacy(hActiveThread, titcontext, avx);

    const CONTEXT& DBGContext = thread->registers.GetContext();
